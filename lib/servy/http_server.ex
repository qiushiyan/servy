defmodule Servy.HttpServer do
  ...
  """
  def start(port \\ 3000) when is_integer(port) and port > 1023 do
    ...
    case :gen_tcp.listen(port, [:binary, packet: :raw, active: false, reuseaddr: true]) do
      {:ok, listen_socket} ->
        if Mix.env() == :dev do
          IO.puts("\n🎧  Listening for connection requests on port http://localhost:#{port}...\n")
        end

        accept_loop(listen_socket)

      {:error, reason} ->
        IO.puts("start server error, #{reason}")
    end
